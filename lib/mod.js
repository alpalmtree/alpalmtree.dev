...
const build = async () => {
  ...
  for (const route of router) {
    const res = await handleRequest(new Request(new URL(route.path, "http://localhost:8000/")));
    ...
    // create folder with pathname to put an index.html file inside
  ...
  /** Build dynamic paths */
  const staticPathsFromDynamic = dynamicPaths.reduce((acc, route) => {
    const params = route.staticPaths()
    ...
    acc.push(...transformedPath)

    return acc
  }, [])

  for (const route of staticPathsFromDynamic) {
    const res = await handleRequest(new Request(new URL(route, "http://localhost:8000/")));
    const content = await res.text()
    Deno.mkdirSync(`${buildPath}/${route}`)
